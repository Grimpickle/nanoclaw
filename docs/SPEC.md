...
Messages must start with the trigger pattern (default: `@Gherkin`):
- `@Gherkin what's the weather?` → ✅ Triggers Claude
- `@andy help me` → ✅ Triggers (case insensitive)
- `Hey @Gherkin` → ❌ Ignored (trigger not at start)
...
[Jan 31 2:35 PM] John: @Gherkin what toppings do you recommend?
```
...
|---------|---------|--------|
| `@Assistant [message]` | `@Gherkin what's the weather?` | Talk to Claude |

### Commands Available in Main Channel Only

| Command | Example | Effect |
|---------|---------|--------|
| `@Assistant add group "Name"` | `@Gherkin add group "Family Chat"` | Register a new group |
| `@Assistant remove group "Name"` | `@Gherkin remove group "Work Team"` | Unregister a group |
| `@Assistant list groups` | `@Gherkin list groups` | Show registered groups |
| `@Assistant remember [fact]` | `@Gherkin remember I prefer dark mode` | Add to global memory |
...
```
User: @Gherkin remind me every Monday at 9am to review the weekly metrics
...
```
User: @Gherkin at 5pm today, send me a summary of today's emails
...
From any group:
- `@Gherkin list my scheduled tasks` - View tasks for this group
- `@Gherkin pause task [id]` - Pause a task
- `@Gherkin resume task [id]` - Resume a paused task
- `@Gherkin cancel task [id]` - Delete a task

From main channel:
- `@Gherkin list all tasks` - View tasks from all groups
- `@Gherkin schedule task for "Family Chat": [prompt]` - Schedule for another group
...
| "No groups registered" | Haven't added groups | Use `@Gherkin add group "Name"` in main |
